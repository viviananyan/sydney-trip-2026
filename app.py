...
# 🔴 Initialize new session states for UI safety
...
with st.expander("➕ Log New Expense", expanded=False):
    with st.form("australia_tracker_form_v11", clear_on_submit=True):
...
if analytics_df.empty:
    ...
    if active_calc_df.empty:
        ...
        if not transactions:
            ...
            # 🔴 THE FIX: Interactive Confirmation Dialogue for Settling
            if not st.session_state.confirm_settle:
                if st.button("✅ Settle All Pending Expenses", use_container_width=True, type="primary"):
                    st.session_state.confirm_settle = True
                    st.rerun()
            else:
                st.warning("⚠️ **Wait!** Are you sure you want to mark all active balances as fully paid? This will permanently update the Google Sheet.")
                col_yes, col_no = st.columns(2)
                
                if col_yes.button("✔️ Yes, Settle It!", type="primary", use_container_width=True):
